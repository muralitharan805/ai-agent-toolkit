...
**Repeat-domain discovery preflight:** Before mining for the same project/domain again, read its accessible existing CSV and compare *underlying actor, task, workflow and root problem* against every previously recorded candidate, including `PARKED` / otherwise closed candidates. Reuse the old ID or skip equivalent problems even when a different title/source describes them; add a new ID only for a genuinely distinct problem. If the prior CSV is unavailable, explicitly say that historical uniqueness could not be checked rather than claiming all proposals are new. Follow [existing-candidate preflight](references/existing-candidate-preflight.md); the writer itself only deduplicates exact IDs.
...
- **One candidate, one row.** Keep IDs stable to avoid duplicates; never overwrite an incompatible or legacy CSV without explicit conversion and backup. A same-ID update needs the complete current candidate data, not just the changed fields.
- **Repeat-domain preflight.** Read existing CSV before proposing more problems in that domain; compare the root problem across different titles/sources and retain solved/parked history. If CSV is unavailable, say semantic de-duplication was not done. Standalone `discovery_csv.py` cannot detect equivalent problems under new IDs.
