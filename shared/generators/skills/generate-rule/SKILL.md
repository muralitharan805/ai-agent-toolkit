...
```bash
python3 shared/generators/skills/generate-rule/scripts/validate_rule.py [determined-toolkit-path]/[rule-name].md
```
...
./bin/sync-context.sh --global [determined-toolkit-path]/rules/[rule-name].md

# Sync to Workspace level (<project>/.agents/rules/)
./bin/sync-context.sh [determined-toolkit-path]/rules/[rule-name].md -w /path/to/project
```
